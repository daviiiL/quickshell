...
- **`modules/`** - Simple versions of Bar, Osd, ScreenCorners, ScreenPaddings, Background
- **`widgets/`** - ClockWidget, PowerIndicator, StatusIcons, Tray, Modal
- **`components/`** - Reusable UI components (CircularProgress, MaterialSymbol, TrayItem, etc.)
- **`utils/`** - Audio, Bluetooth, Brightness, Colors, DateTime, Network, Power, Theme, NiriKeybinds

**`utils/Theme.qml`** - Simple shell theme singleton defining fonts, spacing, rounding, animations

**`utils/Colors.qml`** - Loads color scheme from `colors.json` (Material Design 3 colors)
